...
SELECT comision_emp AS Comisiones 
FROM empleados;
...
-- de “Ciudad Real”
SELECT nombre, e.cargo_emp, d.ciudad
...
SELECT nombre AS 'Nombre del empleado', sal_emp + comision_emp + 500 AS 'Total a pagar (salario + comisión + bonificación de $500)'
FROM empleados
ORDER BY nombre ASC;
...
SELECT nombre AS 'Nombre', sal_emp AS 'Salario', comision_emp AS 'Comisión', sal_emp + comision_emp AS 'Salario total (salario + comisión)'
FROM empleados
...
-- ‘Mantenimiento.
SELECT e.nombre AS 'Nombre del empleado', d.nombre_depto AS 'Departamento'
...
SELECT e.nombre AS 'Nombre del empleado', MAX(e.sal_emp)
FROM empleados e;
...
SELECT MAX(e.sal_emp) AS 'Salario máximo', MIN(e.sal_emp) AS 'Salario mínimo', MAX(e.sal_emp) - MIN(e.sal_emp) AS 'Diferencia'
FROM empleados e;

-- 24. Hallar el salario promedio por departamento.
SELECT d.nombre_depto AS 'Departamento', AVG(e.sal_emp) AS 'Salario promedio'
...
SELECT d.nombre_depto AS 'Departamento', COUNT(e.id_emp) AS 'Cantidad de empleados'
FROM empleados e LEFT JOIN departamentos d ON e.id_depto = d.id_depto
GROUP BY d.nombre_depto
...
SELECT d.nombre_depto AS 'Departamentos sin empleados'
FROM empleados e LEFT JOIN departamentos d ON e.id_depto = d.id_depto
GROUP BY d.nombre_depto
...
ORDER BY d.nombre_depto ASC, e.nombre ASC;